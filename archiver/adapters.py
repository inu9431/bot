...
class GeminiAdapter:
    ...
    def generate_answer(self, prompt: str, image_path: Optional[str] = None) -> str:
        \"\"\"프롬프트를 전달하고 텍스트 응답을 반환

        실제 SDK의 호출 방식은 버전에 따라 다르므로 여기서는 안전하게 lazy 초기화 후
        가능한 SDK 인터페이스를 사용하도록 작성합니다. 통합 시점에 SDK 호출부를
        프로젝트의 genai 사용법에 맞게 교체하세요.
        \"\"\"
        self._setup_client()
        ...
        if hasattr(genai, "Client"):
            try:
                client = genai.Client(api_key=self.api_key) if callable(getattr(genai, "Client")) else genai.Client()
                if hasattr(client, "responses") and hasattr(client.responses, "create"):
                    resp = client.responses.create(model="models/gemini-2.5-flash", input=prompt_text)
        ...
        if hasattr(genai, "responses") and hasattr(genai.responses, "create"):
            try:
                resp = genai.responses.create(model="models/gemini-2.5-flash", input=prompt_text)
...
class NotionAdapter:
    ...
    def create_qna_page(self, dto: QnADTO) -> str:
        ...
        "질문내용": {"rich_text": [{"text": {"content": (dto.question_text or "내용 없음")[:1990]}}]},
        "AI답변": {"rich_text": [{"text": {"content": (dto.ai_answer or "답변 대기 중")[:1990]}}]},
        ...
        try:
            ...
            if response.status_code ==200:
                ...
                raise NotionAPIError(f" 노션 API 에러 :{error_detail.get('message', 'unknown Error')}")
